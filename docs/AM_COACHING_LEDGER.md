...
- **15.3 — Tier-B content PILOT (§10.12 gate) — ✅ DONE (read-only, on `main`, $0.86). VERDICT: ALL 3 DIMENSIONS HOLLOW → STOP. Do NOT scale Tier-B content extraction.** Report: `docs/data-insights/tier_b_pilot_validation_2026-06-16.md`. Scripts `_tier_b_pilot_{sample,score,analyze}.py`.
  ...
  - **Gate (won vs lost, Cliff's δ [95% CI], MWU p):** discovery δ=−0.02 [−0.09,+0.05] p=0.54; consultative δ=−0.01 [−0.10,+0.08] p=0.87; answer_responsiveness δ=+0.02 [−0.13,+0.18] p=0.77. **All negligible, all CIs straddle 0, none significant. Won≈lost everywhere.**
  - **Length confound checked:** scores rise weakly with length (Spearman 0.21–0.48) but stratified within-cell δ ≈ 0 (−0.02/−0.00/+0.04) → no signal beyond OR from length.
  - **WHY (scorer trustworthy — spot-check §10.13 confirms calibration, NOT default-to-1):** correctly finds rare real instances (Ehab "Do you have a budget in mind" → discovery 3; Linda "has to be 4pp sections for the folding" → consultative 4; precise spec answer → responsiveness 5; "Thanks Karl" → 1). The **floor is REAL**: quote-linked email is transactional (quotes/artwork/proofs/approvals/invoices); discovery & consultative barely occur in won OR lost. Some MOST-consultative threads were LOST; many WON were pure logistics.
  - **Key limitation (honest):** quote-linkage selects EXECUTION threads; consultative *selling* likely lives in PRE-quote threads (not quote-linked) or off-email (phone). Proves content quality doesn't predict winning *in the measurable quote-linked email channel* — not that consultative selling is irrelevant. Optional future redesign: target pre-quote/early-relationship threads, re-run the same cheap gate before scaling.
  - **Per-dimension:** discovery DROP (floored), consultative DROP (floored), answer_responsiveness do-not-ship-as-win-signal (no outcome link; also caveated by 15.3b mis-pairs + one-line-ack framing). **Lead coaching with Tier-A structural (15.2), self-referential.**
...
- **15.3c — Tracker pairing fix (canonical thread + pair validation) — ✅ DONE (on `main`, §10.14 fix-the-tap).** 4-step fix applied to `response_time_tracker.py` (durable, also fixes the live pipeline): (1) group/pair within `canonical_thread_id` not provider `thread_id`; (2) validate each pair (`_subjects_match` OR `_reply_addressed_back` — reply addressed to the original sender); (3) exclude auto-reply/OOO + automated/no-reply senders (`AUTOMATED_SENDER_PATTERNS`) from ANCHORING a pair (the responding-to side, which `is_auto_reply` never checked); (4) recompute. Pipeline-safe: `orchestrator._assign_canonical_threads` (step 9) runs before the engagement step (10), so `canonical_thread_id` is populated live.
  ...
  - Repro/ops scripts kept: `_resp_latency_plausibility.py`, `_recompute_response_metrics_15_3c.py`, `_cleanup_hello_15_3c.py`, `_verify_mailbox_scoped.py`, `_verify_crosscanonical.py`, `_verify_am_metric_final.py`.

- **15.4 — AM-facing frontend (self-view) — 🔜 (after 15.2, MVP surface).** React view, each AM sees their OWN patterns. Simpler — no cross-AM permission logic. The MVP-ready surface.
