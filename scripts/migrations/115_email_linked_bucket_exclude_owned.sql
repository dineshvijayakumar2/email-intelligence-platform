-- Migration 115: Tighten the "Has email link (fixable)" bucket
--
-- The email_linked bucket in qb_cleanup_dashboard_stats (107) counted every
-- unmatched QB customer that had an email -> contact -> company chain. But if the
-- resolved company is ALREADY owned by another QB customer (matched_company_id
-- back-reference), the chain is NOT actionable as a simple link: the 1:1 match
-- constraint is already satisfied. Those rows are QB-side duplicates ("DUP_QB")
-- or persons who merely share a contact email with an unrelated, correctly-owned
-- company -- they inflated the "fixable" count (~20 of ~26).
--
-- Fix: only count a chain whose target company is NOT already matched by any QB
-- customer. Genuine cross-wires (company owned by an UNRELATED qb) are repaired
-- by displacement, not by this bucket, so excluding owned companies is correct.
--
-- Only the email_linked CTE changes; the rest of the function is unchanged.
...
BEGIN
    ...
    -- EXCLUDING chains whose target company is already owned by another QB
    -- customer (those are duplicates / shared-email persons, not linkable).
    email_linked AS (
        SELECT
            COUNT(DISTINCT qc.id)::int AS cnt,
            COALESCE(SUM(DISTINCT qc.total_invoiced), 0) AS revenue
        ...
        WHERE qc.client_id = p_client_id
          AND qc.matched_company_id IS NULL
          AND NOT EXISTS (
              SELECT 1 FROM qb_customers qc3
              WHERE qc3.client_id = qc.client_id
                AND qc3.matched_company_id = cc2.customer_company_id
          )
    ),
